        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-md">
            <div className="nd-glass p-8 sm:p-10">
              <div className="text-center mb-8"><h1 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-2">Create Account</h1><p className="text-slate-500 dark:text-zinc-400 text-sm">Start your premium banking journey in seconds</p></div>
              {err && <div className="mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">{err}</div>}
              <form onSubmit={submit} className="space-y-4">
                {fields.map(f => (
                  <div key={f.key}><label className="block text-xs font-bold uppercase tracking-[0.15em] text-slate-400 dark:text-zinc-500 mb-2">{f.label}</label><div className="relative"><f.icon className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-zinc-600"/><input type={f.type} value={(form as any)[f.key]} onChange={up(f.key)} required placeholder={f.ph} className="nd-input pl-11"/></div></div>
                ))}
              ...
              <p className="mt-8 text-center text-sm text-slate-400 dark:text-zinc-500">Already have an account? <Link to="/login" className="text-amber-500 dark:text-amber-400 font-bold hover:text-amber-400 dark:hover:text-amber-300">Sign in</Link></p>
            </div>
          </div>
        </div>
      </div>
