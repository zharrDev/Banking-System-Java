...
export const Footer: React.FC = () => {
  ...
  return (
    <footer
      className="py-16 border-t border-amber-500/5 bg-gradient-to-b from-nd-bg to-nd-card/50 px-6 sm:px-8 lg:px-12 dark:from-nd-bg dark:to-nd-card/50"
      role="contentinfo"
    >
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-12 md:grid-cols-4">
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-zinc-400 text-base leading-relaxed max-w-xs dark:text-zinc-300">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {features.map((f, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 dark:bg-white/5 dark:hover:bg-white/10">
                  <div className="flex-shrink-0 p-2 rounded-lg bg-amber-500/10 text-amber-400">
                    <f.icon className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="font-semibold text-white text-sm dark:text-white">{f.title}</p>
                    <p className="text-zinc-500 text-xs dark:text-zinc-400">{f.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Navigation Links */}
          <div>
            <h4 className="font-bold text-white mb-4 dark:text-white">Product</h4>
            <ul className="space-y-3">
              {footerLinks.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-sm text-zinc-400 hover:text-amber-400 transition-colors duration-300 relative group dark:text-zinc-400 dark:hover:text-amber-400"
                  >
                    {link.label}
          ...
          {/* Company Links */}
          <div>
            <h4 className="font-bold text-white mb-4 dark:text-white">Company</h4>
            <ul className="space-y-3">
          ...
          {/* Social & Legal */}
          <div>
            <h4 className="font-bold text-white mb-4 dark:text-white">Connect</h4>
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    className="p-2 rounded-xl bg-white/5 hover:bg-amber-500/10 border border-white/5 hover:border-amber-500/30 transition-all duration-300 hover:scale-110 hover:rotate-6 dark:bg-white/5 dark:hover:bg-amber-500/10"
                  >
                    <Icon className="h-4 w-4 text-zinc-400 hover:text-amber-400 transition-colors duration-300 dark:text-zinc-400 dark:hover:text-amber-400" />
                  </a>
                );
              })}
            </div>
